    ...
    for rx in DATE_RES:
        ...
        try:
            if len(p[0]) == 4:
                ...
                mo, d, y = int(p[0]), int(p[1]), int(p[2])
                if y < 100:
                    y += 2000
...
<title>ReceiptJSON — invoice/receipt parser API</title>
<style>body{font-family:system-ui,Segoe UI,Roboto,Arial;max-width:920px;margin:32px auto;padding:0 16px;line-height:1.5}code,pre{background:#f6f8fa;padding:2px 6px;border-radius:6px}pre{padding:12px;overflow:auto}.card{border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin:14px 0}.muted{color:#4b5563}</style>
</head><body>
<h1>ReceiptJSON</h1>
<p class='muted'>Minimal, privacy-forward API that converts invoices/receipts into structured JSON (text-only MVP).</p>
<div class='card'><h2>API</h2><ul><li><code>GET /health</code></li><li><code>POST /parse</code></li><li>Docs: <a href='/docs'>/docs</a></li></ul></div>
<div class='card'><h2>Pricing suggestion</h2><ul><li><b>Free</b>: 25 parses/month</li><li><b>Starter</b>: $9/mo for 1,000 parses</li><li><b>Pro</b>: $49/mo for 10,000 parses</li></ul><p class='muted'>Payments not enabled in this demo.</p></div>
<div class='card'><h2>Data handling</h2><ul><li>No document storage by default.</li><li>Logs minimal metadata only (no raw content).</li></ul></div>
<div class='card'><h2>Contact</h2><p>Email: <code>support@example.com</code></p></div>
<div class='card'><h2>Try it</h2><pre>curl -s http://localhost:8000/parse \\
  -F \"text=ACME Corp\\nInvoice #INV-1001\\n2026-02-28\\nSubtotal $10.00\\nTax $0.80\\nTotal $10.80\" | python -m json.tool</pre></div>
<p class='muted'>Disclaimer: best-effort extraction; verify before bookkeeping.</p>
</body></html>"""
    )
...
async def parse_endpoint(
    ...
    meta = {
        ...
        "note": "MVP parses plain text only; PDF/image require OCR/PDF extraction in a paid tier.",
    }
