...
def parse_invoice_text(text: str) -> Dict[str, Any]:
    ...
    total = next((v for v in (_money_to_float(c) for c in total_candidates) if v is not None), None)
    taxes = next((v for v in (_money_to_float(c) for c in tax_candidates) if v is not None), None)
